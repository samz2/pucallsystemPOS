...
mhnUI = {
    ...
    lock: function() {
        mhnUI.page.hide(), pattern = new PatternLock(".mhn-lock", {
            margin: 15
        }), $(".mhn-lock-title").html($(".mhn-lock-title").data("title")), pattern.checkForPattern("0", function() {
            console.warn(pattern.getPattern()),
            ...
            $(".mhn-lock-title").html('<span class="mhn-lock-success">Yes! you unlocked pattern</span>'), $(".patt-holder").addClass("patt-success"), setTimeout(function() {
                pattern.reset(), mhnUI.message()
            }, 1e3), $("#patronModal").modal("hide"), $("#pin").val(pattern.getPattern())
        })  
    },
    message: function() {
