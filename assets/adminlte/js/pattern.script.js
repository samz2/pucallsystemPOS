...
function login() {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data == "inicio" || data == "cocina" || data=="venta") {
...
function ingresar(usuario) {
    // $(".ui-users").toggleClass("users-active");
    $(".ui-lock").toggleClass("lock-active");
    $("#user-lock").text($("#nombre_completo" + usuario).text());
    $(".ui-container-users").toggleClass("ui-container-users-active");
    $("#codigo").val(usuario);
    mhnUI.lock();
}
mhnUI = {
    pattern: "",
    setup: function () {
        this.filter(), this.colors(), this.links.setup(), this.dialog.setup(), setInterval("mhnUI.datetime()", 1e3)
    },
    api: function (handleData) {
        $.ajax({
            url: $("#url").val() + "/login_pattern/" + $("#codigo").val(),
            type: "POST",
            dataType: "json",
            // async: false,
            success: function (data) {
                handleData(data);
                $(".user-lock-p").text(data.nombre);
                if (data.inputpassword == "1") {
                    $(".ui-container-pt").hide();
                    $(".ui-container-ip").fadeIn(1000);
                    $("input[name='usuario']").val(data.usuario);
                } else {
                    $(".ui-container-ip").hide();
                    $(".ui-container-pt").show();
                }
            }
        });
    },
    lock: function () {
        pin = mhnUI.api(function (output) {
            return output['pin'];
        });
        mhnUI.page.hide(), pattern = new PatternLock(".mhn-lock", {
            margin: 15
        }), $(".mhn-lock-title").html($(".mhn-lock-title").data("title")),
            mhnUI.api(function (output) {
                pattern.checkForPattern(output['pin'], function () {
                    $(".mhn-lock-title").html('<span class="mhn-lock-success">Si! patron correcto</span>'), $(".patt-holder").addClass("patt-success"), setTimeout(function () {
                        pattern.reset(), mhnUI.message()
                    }, 1e3), mhnUI.page.show(),  window.location.href = $("#url").val() + "/login/" + $("#codigo").val()
                }, function () {
                    $(".mhn-lock-title").html('<span class="mhn-lock-failure">Opps! patron incorrecto</span>'), $(".patt-holder").removeClass("patt-success"), setTimeout(function () {
                        pattern.reset(), mhnUI.message()
                    }, 2e3)
                })
            });
    ...
    datetime: function () {
        var t = new Array("Domingo", "Lunes", "Martes", "Miércoles", "Jueves", "Viernes", "Sábado"),
            e = new Array("Enero", "Febrero", "Mazo", "Abril", "Mayo", "Junio", "Julio", "Agosto", "Septiembre", "Octubre", "Noviembre", "Diciembre"),
            n = new Date,
            i = n.getYear();
        1e3 > i && (i += 1900);
        var a = n.getDay(),
