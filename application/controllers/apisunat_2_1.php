...
class Apisunat
{
    ...
    {
        return $this->CI->db->where('id', $id)->get('producto')->row();
    }
    ...
    public function crear_xml_factura($cabecera, $detalle, $ruta)
    {
        ...
        foreach ($detalle as $value) {
            ...
            $xmlCPE .= '<cac:InvoiceLine>
                    ...
                    <cbc:PriceAmount currencyID="' . $cabecera["COD_MONEDA"] . '">' . $value->precio . '</cbc:PriceAmount>
                    <cbc:PriceTypeCode listName="Tipo de Precio" listAgencyName="PE:SUNAT" listURI="urn:pe:gob:sunat:cpe:see:gem:catalogos:catalogo16">' . $txtPRECIO_TIPO_CODIGO . '</cbc:PriceTypeCode>
                    </cac:AlternativeConditionPrice>
                    </cac:PricingReference>
                    <cac:TaxTotal>
                    <cbc:TaxAmount currencyID="' . $cabecera["COD_MONEDA"] . '">' . $igv . '</cbc:TaxAmount>
                    <cac:TaxSubtotal>
                    <cbc:TaxableAmount currencyID="' . $cabecera["COD_MONEDA"] . '">' . $value->subtotal . '</cbc:TaxableAmount>
                    <cbc:TaxAmount currencyID="' . $cabecera["COD_MONEDA"] . '">' . $igv . '</cbc:TaxAmount>
                    <cac:TaxCategory>
                    <cbc:ID schemeID="UN/ECE 5305" schemeName="Tax Category Identifier" schemeAgencyName="United Nations Economic Commission for Europe">E</cbc:ID>
                    <cbc:Percent>' . $cabecera["POR_IGV"] . '</cbc:Percent>
                    <cbc:TaxExemptionReasonCode listAgencyName="PE:SUNAT" listName="SUNAT:Codigo de Tipo de Afectación del IGV" listURI="urn:pe:gob:sunat:cpe:see:gem:catalogos:catalogo07">' . $txtCOD_TIPO_OPERACION . '</cbc:TaxExemptionReasonCode>
                    <cac:TaxScheme>
                    <cbc:ID schemeID="UN/ECE 5153" schemeAgencyID="6">9997</cbc:ID>
                    <cbc:Name>EXO</cbc:Name>
                    <cbc:TaxTypeCode>VAT</cbc:TaxTypeCode>
    ...
    public function crear_xml_nota_credito($cabecera, $detalle, $ruta)
    {
        ...
        foreach ($detalle as $key => $value) {
            ...
            $txtIGV = 0;
            $txtCOD_TIPO_OPERACION = '20';
            $producto = $this->get($value->producto, 'producto');
            $marca = $this->get($producto->marca, 'marca');
            $nombre = trim($producto->nombre) . ' ' . ($marca ? $marca->nombre : '');
            $xmlCPE .= '
            <cac:CreditNoteLine>
                    <cbc:ID>' . ($key + 1) . '</cbc:ID>
                    <cbc:CreditedQuantity unitCode="NIU">' . $value->cantidad . '</cbc:CreditedQuantity>
                    <cbc:LineExtensionAmount currencyID="' . $cabecera["COD_MONEDA"] . '">' . $subtotal . '</cbc:LineExtensionAmount>

                    <cac:PricingReference>
                    <cac:AlternativeConditionPrice>
                    <cbc:PriceAmount currencyID="' . $cabecera["COD_MONEDA"] . '">' . $value->precio . '</cbc:PriceAmount>
                    <cbc:PriceTypeCode>' . $txtPRECIO_TIPO_CODIGO . '</cbc:PriceTypeCode>
                    </cac:AlternativeConditionPrice>
                    </cac:PricingReference>

                    <cac:TaxTotal>
                    <cbc:TaxAmount currencyID="' . $cabecera["COD_MONEDA"] . '">' . $txtIGV . '</cbc:TaxAmount>
                    <cac:TaxSubtotal>
                    <cbc:TaxableAmount currencyID="' . $cabecera["COD_MONEDA"] . '">' . $subtotal . '</cbc:TaxableAmount>
                    <cbc:TaxAmount currencyID="' . $cabecera["COD_MONEDA"] . '">' . $txtIGV . '</cbc:TaxAmount>
                    <cac:TaxCategory>
                    <cbc:Percent>' . $cabecera["POR_IGV"] . '</cbc:Percent>
                                         <cbc:TaxExemptionReasonCode>' . $txtCOD_TIPO_OPERACION . '</cbc:TaxExemptionReasonCode>
                                         <cac:TaxScheme>
                                            <cbc:ID>1000</cbc:ID>
                                            <cbc:Name>IGV</cbc:Name>
                                            <cbc:TaxTypeCode>VAT</cbc:TaxTypeCode>
                    ...
                    <cac:Item>
                        <cbc:Description>
                            <![CDATA[' . $validacion->replace_invalid_caracters($nombre) . ']]>
                        </cbc:Description>
                        <cac:SellersItemIdentification>
                            <cbc:ID>
                                <![CDATA[' . $validacion->replace_invalid_caracters($producto->codigo) . ']]>
                            </cbc:ID>
                        </cac:SellersItemIdentification>
                     </cac:Item>
